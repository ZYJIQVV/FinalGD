...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(
            f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date gd
            'default.yaml' file.\nPlease update your code with 'pip install -U gd' and if necessary replace
...
class ThreadingLocked:
    ...
    Example:
        ```python
        from gd.utils import ThreadingLocked
...
class SettingsManager(dict):
    ...
    def __init__(self, file=SETTINGS_YAML, version="0.0.4"):
        ...
        from gd.utils.checks import check_version
        from gd.utils.torch_utils import torch_distributed_zero_first
        ...
        with torch_distributed_zero_first(RANK):
            ...
            if not (correct_keys and correct_types and correct_version):
                LOGGER.warning(
                    ...
                    f"with your settings or a recent gd package update. {help_msg}"
                )
                self.reset()
...
def deprecation_warn(arg, new_arg, version=None):
    ...
    LOGGER.warning(
        f"WARNING ⚠️ '{arg}' is deprecated and will be removed in 'gd {version}' in the future. "
