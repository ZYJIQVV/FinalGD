...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Args:
        ...
        package (str, optional): Python package to use instead of requirements.txt file, i.e. package='gd'.
    ...
    Example:
        ```python
        from gd.utils.checks import parse_requirements

        parse_requirements(package='gd')
        ```
    """
...
def check_version(
    ...
    if not current:  # if current is '' or None
        ...
        return True
    elif not current[0].isdigit():  # current is package name rather than version string, i.e. current='gd'
        try:
...
def check_latest_pypi_version(package_name="gd"):
    """
...
def check_pip_update_available():
    """
    Checks if a new version of the gd package is available on PyPI.
        ...
        with contextlib.suppress(Exception):
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(
                    ...
                    f"Update with 'pip install -U gd'"
                )
                return True
    return False
...
def check_requirements(requirements=ROOT.parent / "requirements.txt", exclude=(), install=True, cmds=""):
    ...
    Example:
        ```python
        from gd.utils.checks import check_requirements

        # Check a requirements.txt file
        check_requirements('path/to/requirements.txt')

        # Check a single package
        check_requirements('gd>=8.0.0')

        # Check multiple packages
        check_requirements(['numpy', 'gd>=8.0.0'])
        ```
    """
...
def check_yolo(verbose=True, device=""):
    ...
    from gd.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from gd.utils import ENVIRONMENT, IS_GIT_DIR
    from gd.utils.torch_utils import get_cpu_info
    ...
    for r in parse_requirements(package="gd"):
        try:
...
def check_amp(model):
    ...
    Example:
        ```python
        from gd import YOLO
        from gd.utils.checks import check_amp
