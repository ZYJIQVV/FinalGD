...
from gd.utils import LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file

# Define Ultralytics GitHub assets maintained at https://github.com/ultralytics/assets
GITHUB_ASSETS_REPO = "gd/assets"
GITHUB_ASSETS_NAMES = (
...
def delete_dsstore(path, files_to_delete=(".DS_Store", "__MACOSX")):
    ...
    Example:
        ```python
        from gd.utils.downloads import delete_dsstore
...
def zip_directory(directory, compress=True, exclude=(".DS_Store", "__MACOSX"), progress=True):
    ...
    Example:
        ```python
        from gd.utils.downloads import zip_directory
...
def unzip_file(file, path=None, exclude=(".DS_Store", "__MACOSX"), exist_ok=False, progress=True):
    ...
    Example:
        ```python
        from gd.utils.downloads import unzip_file
...
def get_google_drive_file_info(link):
    ...
    Example:
        ```python
        from gd.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    Example:
        ```python
        from gd.utils.downloads import safe_download
...
def get_github_assets(repo="gd/assets", version="latest", retry=False):
    """
    Retrieve the specified version's tag and assets from a GitHub repository. If the version is not specified, the
    function fetches the latest release assets.

    Args:
        repo (str, optional): The GitHub repository in the format 'owner/repo'. Defaults to 'gd/assets'.
    ...
    Example:
        ```python
        tag, assets = get_github_assets(repo='gd/assets', version='latest')
        ```
    """
...
def attempt_download_asset(file, repo="gd/assets", release="v8.2.0", **kwargs):
    """
    ...
    Example:
        ```python
        file_path = attempt_download_asset('yolov5s.pt', repo='gd/assets', release='latest')
        ```
    """
    from gd.utils import SETTINGS  # scoped for circular import
